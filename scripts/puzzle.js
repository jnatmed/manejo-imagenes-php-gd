...
var restart=0;
function won(){
	ctx.clearRect(300,300,150,150); // LIMPIA SECTOR `I`
	var img=document.getElementById("puzz9");
	var pat=ctx.createPattern(img,"repeat");
	ctx.fillStyle=pat;
	ctx.fillRect(300,300,150,150); // LIMPIA SECTOR `I`
...
function enviarInfo(movimiento){
	...
	xhttp.open("POST", "movimientosData.php?movimiento=" + movimiento);
	// xhttp.open("POST");
	xhttp.send(movimiento);
}
