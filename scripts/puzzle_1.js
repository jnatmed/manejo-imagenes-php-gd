/**
 * tomo el elemento canvas
 * se lo sumo al contexto `cx`
 * inicializo variable  
 *    `empty` => en 9 , vacio en 9 
 * 	  `moves` => -1 , movimientos en -1: debe significar que aun 
...
var moves=-1;
/**
 * shuffle => barajar, entiendo que significa mezclar
 * el arreglo de elementos
 * tiene como entrada `array`
 * debe cuando llama la funcion shuffle([1,2,3,4,5,6,7,8,0])
 * y la asigna a `im` 
 */
...
/**
 *  `ar` viene a ser el arreglo, que abreviatura poco intuitiva :/
 *  `im` viene a ser el aareglo barajado
 */	
...
function enviarInfo(movimiento){
    $.ajax({
        url : 'controllers/movimientosData.php',
        type : 'POST',
        async : true,
        data : {
            action : movimiento,
